...
var localFontDir = path.join(getRunTimeDirectory(), '/fonts');
var configOptions = {};
configOptions.removeFonts = false;
var platform = os.platform();
var sysFontDir = 'c:/Windows/Fonts';
var invokeCallBack;
var _invokeCallBack = function (callback, error) {
    //reset font counts
    fontCount = 0;
    currentFontCount = 0;
...
function getRunTimeDirectory() {
    //account for the fact that vbs files cannot run inside an asar file.  Rewrite the path if the reference is found.
    let path = __dirname;
...
module.exports = function (fontPath, callback, options) {
    ...
    if (options && options.overrideLocalFileDir) {
        localFontDir = options.overrideLocalFileDir + "\\fonts";
...
function tryInstallFonts(fontPathDir) {
    if (fontCount === currentFontCount) {
        ...
        var addFont = exec(
            'cscript ' +
            JSON.stringify(path.join(getRunTimeDirectory(), 'addFontInDirectory.vbs')) +
            ' ' + JSON.stringify(localFontDir),
            (error, stdout, stderr)=>{
                cleanFontDirectory(fontPathDir);
                if(error){
                    return invokeCallBack(error);
                }else{
...
function tryInstallFontFile(fontFilePath) {
    ...
    var addFont = exec(
        'cscript ' + 
        JSON.stringify(path.join(getRunTimeDirectory(), 'addFontFile.vbs')) + 
        ' "' + JSON.stringify(fontFilePath) + '"',
        (error, stdout, stderr)=>{
            cleanFontDirectory(fontFilePath, true);
            if(error){
                return invokeCallBack(error);
